...
export function contaminantForCode(code) {
  return Object.values(CONTAMINANTS).find((item) => item.codes.includes(String(code)));
}
...
export function toDisplayUnit(value, fromUnit, definition) {
  ...
  if (definition.unit === 'µg/L' && (unit === 'ng/l' || unit === 'ppt')) return n / 1000;
  if (definition.unit === 'mg/L' && (unit === 'µg/l' || unit === 'ug/l')) return n / 1000;
  return n;
}
